...
Both pieces are plain containers, so any container host works. A simple, low-cost path:

1. **API** (`server/`): push the image to a registry and deploy to [Fly.io](https://fly.io), [Render](https://render.com), or [Railway](https://railway.app) — all three can build directly from `server/Dockerfile`. Set `JWT_SECRET` (long random value) and `CLIENT_ORIGIN` (the deployed frontend's URL, for CORS) as environment variables on the service. Mount a persistent volume at the path given by `DB_PATH` so the SQLite file survives redeploys (Fly volumes / Render disks both support this); for a fully managed alternative, swap `better-sqlite3` for a hosted Postgres and a driver like `pg` — the service layer already isolates all SQL behind `createAuthService`/`createTasksService`, so only those two files and `db.ts` would need to change.
2. **Client** (`client/`): `npm run build --workspace client` produces static assets in `client/dist/`; deploy them to [Vercel](https://vercel.com) or [Netlify](https://www.netlify.com) (drag-and-drop or `vercel deploy` / `netlify deploy`), or serve them from the same `client/Dockerfile` nginx image on the same host as the API. Point the client at the deployed API by setting the proxy target (or switching `api/client.ts`'s `API_BASE` to an absolute URL) to the API's public URL.
...
- CI (GitHub Actions) running lint/typecheck/test on every PR, and a Dependabot/renovate config for dependency updates.
- Swapping SQLite for a managed Postgres instance for real multi-instance deployments (SQLite's single-writer model is fine for this exercise but not for horizontal scaling).
